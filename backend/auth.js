...
async function register(req, res) {
  ...
  const hash = crypto.pbkdf2Sync(password, salt, numHashIterations, hashLengthvi , "sha512").toString("hex");

  const query = 'INSERT INTO users (firstName, lastName, username, email, hash, salt, role) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id';

  const values = [firstName, lastName, username, email, hash, salt, role];
