async function DisplayTable(){
    try{
        ...
        const response = await fetch(`/api/workout?limit=${limit}`, {credentials: "include"});
...
async function TruncateTable(){
   try {
        const response = await fetch("/api/workout/truncate", {
            method: "DELETE",
            credentials: "include"
        });
...
async function PopulateTable(event) {
    console.log("here we are in workouttablepage.js");
    ...
    try {
        const response = await fetch("/api/workout/populate", {
            method: "POST",
        ...
        if (response.ok) {
            ...
            if(result.success == true){
               alert(`${result.message} for ${result.workoutname}`);
               window.location.href = "workouttablepage.html";
            } else {
